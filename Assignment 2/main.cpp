...
enum TokenType{
                /// declaring new data types in the enumerator
...
const char* TokenTypeStr[]=
            {
                 /// add the data types in the array
                "Int", "Real", "Bool",
                "If", "Then", "Else", "End", "Repeat", "Until", "Read", "Write",
...
{
      /// Add the new data type as a reserved words
    Token(INTEGEr, "int"),
    Token(REAl, "real"),
    Token(BOOLEAn, "bool"),
...
// the new grammar rule for the data type
// datatypestmt -> (int |bool |real) identifier
...
enum NodeKind{
                ...
                /// add new Node Kind type int the enumerator
                DATA_TYPE_NODE
             };
...
const char* NodeKindStr[]=
            {
                ...
                "Oper", "Num", "ID", "DataType" /// add new Node Kind type
            };

/// add the new Data Type in the enumerator
...
/**
    the grammar rule to programing function
    the new grammar rule for the data type
    datatypestmt -> (int |bool |real) identifier
*/
TreeNode* datatypestmt(CompilerInfo* pci, ParseInfo* ppi)
{
      /// the Start DataTypeStmt
    pci->debug_file.Out("Start DataTypeStmt");

    /// new node in the tree
    TreeNode* tree=new TreeNode;
    /// While next_token = integer | real | boolean
    while(ppi->next_token.type==INTEGEr || ppi->next_token.type==REAl
           || ppi->next_token.type==BOOLEAn)
    {
          /// the kind of the tree
    tree->node_kind=DATA_TYPE_NODE;

    /// Check the next_token type
    if(ppi->next_token.type==INTEGEr )
      tree->expr_data_type=INTEGER;
    else if(ppi->next_token.type==REAl)
      tree->expr_data_type=REAL;
    else if(ppi->next_token.type==BOOLEAn)
      tree->expr_data_type=BOOLEAN;

    //tree->type = ppi->next_token.type;
    /// git the line number
    tree->line_num=pci->in_file.cur_line_num;
    //tree->child[0]=tree->id;
      /// Match next token type
    Match(pci, ppi,  ppi->next_token.type);
    if(ppi->next_token.type==ID)
            /// Allocate And Copy the id to the str array
         AllocateAndCopy(&tree->id, ppi->next_token.str);
      //Check ID
    Match(pci, ppi, ID);
    }
      ///the end of Data Type Statement
    pci->debug_file.Out("End DataTypeStmt");
    return tree;
}
...
TreeNode* Stmt(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    /// When next_token type = Data type call the function
    else if(ppi->next_token.type==INTEGEr) tree=datatypestmt(pci, ppi);                     /// ////////////////////////////////////////////
    else if(ppi->next_token.type==BOOLEAn) tree=datatypestmt(pci, ppi);
    else if(ppi->next_token.type==REAl) tree=datatypestmt(pci, ppi);                        /// ////////////////////////////////////////////
    else throw 0;
...
void PrintTree(TreeNode* node, int sh=0)
{
    ...
    if(node->node_kind==OPER_NODE)
        ...
        printf("[%d]", node->num);
         ...
         /// to print the DATA_TYPE_NODE's ID
    if(node->node_kind==DATA_TYPE_NODE)
        printf("[%s]", node->id);

        /// to print the DATA_TYPE of the id
...
void Analyze(TreeNode* node, SymbolTable* symbol_table)
{
    ...
    if(node->node_kind==ID_NODE || node->node_kind==READ_NODE || node->node_kind==ASSIGN_NODE)
        ...
        /** DATA_TYPE_NODE */
    if(node->node_kind == DATA_TYPE_NODE)
          /// insert id and lint num of the node to the symbol_table
          symbol_table->Insert(node->id, node->line_num);
...
/*
    if(node->node_kind==OPER_NODE)
          /// Error message for non writ data type
        if(node->child[0]->expr_data_type!=INTEGER || node->child[1]->expr_data_type!=INTEGER
           || node->child[0]->expr_data_type!=REAL || node->child[1]->expr_data_type!=REAL)
